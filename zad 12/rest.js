...
function _list() {
   ...
   request.open("GET", "http://pascal.fis.agh.edu.pl/~9kolodziejmir/lab12/rest/list", true);
   request.send(null);
}
...
function _insert(form)  {
    ...
    request.open("POST", "http://pascal.fis.agh.edu.pl/~9kolodziejmir/lab12/rest/save", true);
    request.send(txt);
}
...
print (id_mongo);
    request.open("DELETE", "http://pascal.fis.agh.edu.pl/~9kolodziejmir/lab12/rest/delete1/"+id_mongo, true);
...
function _update(form) {
    ...
    request.open("PUT", "http://pascal.fis.agh.edu.pl/~9kolodziejmir/lab12/rest/update1/"+id_mongo, true);
    request.send(txt);
}

function _search_form() {
   var form1 = "<form><table>";
   form1 += "<tr><td>lname</td><td><input type='text' name='lname' value='' ></input></td></tr>";
   form1 += "<tr><td></td><td><input type='button' value='Wyślij' onclick='_search(this.form)'></input></td></tr>";
   form1 += "</table></form>";
   document.getElementById('data').innerHTML = form1;
   document.getElementById('result').innerHTML = '';
}
...
function _search(form) {
   ...
   request.onreadystatechange = function () {
      if (request.readyState == 4) {
         objJSON = JSON.parse(request.response);
         for (var id in objJSON) {
            if ( objJSON[id]['lname'] == lname && id!='undefined' ) {
               for (var prop in objJSON[id]) {
                  if (prop == '_id') 
                  { 
                     answer += "<p>ident: " + objJSON[id][prop]['$oid'] + ";"; 
                  }
                  else if (prop == 'year')
                  {
                     answer += prop + ": " + objJSON[id][prop] + ", </p>";
                  }
                  else 
                  { 
                     answer += prop + ": " + objJSON[id][prop] + ", "; 
                  }
               }
            }
         }
   ...
   request.open("GET", "/~9kolodziejmir/lab12/rest/list/", true);
   request.send(null);
}
